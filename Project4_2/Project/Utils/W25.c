...
void SPI1_Init(void) {
    ...
    GPIO_SetBits(GPIOB, GPIO_Pin_0);// 先不进行片选
...
// 获取地址, 检测当前W25Q64的末尾存储位置
void getEndADDRESS(int * w25AddressEnd){
	
	int endAddress = W25_END_ADDRESS;
	int firstAddress = 0x000000;
	while((endAddress - firstAddress ) > 0x000100){
		  
			int currentAddress = (endAddress-firstAddress)/2 + firstAddress;
	    uint8_t num1 = W25Q64_Read_Byte(currentAddress);
				
			if((num1&0xFF) == 0xFF){ // 已经是末尾之后了
	...
	while(1){
		 uint8_t num1 = W25Q64_Read_Byte(firstAddress);	
		if((num1&0xFF) == 0xFF){ // 已经是末尾之后了
		   break;
		}else{ // 结尾之前
...
void saveSensorToW25(int  w25AddressEnd){
        ...
        // 读取传感器数据
       	uint16_t AD1 = AD_GetValue(ADC_Channel_1);		
				...
				// 转换为字符串并追加到buf
				char temp[50] = {0};
				sprintf(temp, "=%s:%u,%s:%u=","1", AD1, "2", AD4);
		
				W25Q64_Write_String(w25AddressEnd, temp);
				w25AddressEnd += strlen(temp);
